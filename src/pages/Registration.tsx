...
import { LogIn, UserPlus, Camera, Check, X, Key, Mail, Loader2 } from 'lucide-react';
...
export const Registration: React.FC = () => {
  ...
  const handleCompleteRegistration = async () => {
    ...
    try {
      setLoading(true);
      console.log('Registration: Completing with data:', formData);
      const uid = user.uid;
      ...
      const profileData = {
        ...
        sex: formData.sex, // Save 'M' or 'F' as per the schema
      ...
      if (result.success || result.uid) {
        ...
        console.log('Registration: Profile refreshed');
        setLoading(false);
      } else {
  ...
  function renderAuth() {
    return (
      <motion.div 
        ...
        className="bg-card-dark border border-border-dark p-8 rounded-2xl shadow-2xl w-full max-w-md mx-auto"
      >
        ...
        <form onSubmit={handleAuth} className="space-y-4">
            ...
            >
                {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : (authMode === 'login' ? 'Увійти' : 'Створити акаунт')}
            </button>
        </form>
      ...
      >
        <div className="bg-card-dark border border-border-dark p-8 rounded-2xl shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex gap-2">
                  {(['M', 'F'] as const).map((gen) => (
                    <button
                      key={gen}
                      onClick={() => setFormData({...formData, sex: gen})}
                ...
                >
                  {formData.passportPhoto ? (
                    <>
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity" title="Змінити фото">
          ...
          <div className="mt-8">
            ...
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Далі'}
            </button>
          </div>
        </div>
      </motion.div>
        ...
        <div className="flex flex-col items-center gap-8">
          ...
          <div className="w-full space-y-4">
            ...
            <button 
              onClick={() => setStep(2)}
              disabled={loading}
              className="w-full py-3 text-[10px] uppercase font-black tracking-widest text-text-muted hover:text-white transition-colors"
            >
              Редагувати дані
            </button>
          </div>
    ...
    <div className="h-[100dvh] bg-[#0A0A0C] flex flex-col items-center justify-start overflow-y-auto p-4 md:justify-center text-[#E0E0E0]">
            ...
            {dbStatus.connecting ? (
              <div className="flex flex-col items-center">
                ...
                <p className="text-[10px] text-text-dim uppercase tracking-widest leading-loose max-w-sm">
                    Це може зайняти до 60 секунд під час першого запуску сервера.
                </p>
              </div>
            ) : (
              <>
                ...
                <button 
                    ...
                    className="w-full py-5 bg-white text-black font-black uppercase tracking-widest rounded-3xl"
                >
                    ПОВТОРИТИ СПРОБУ
                </button>
              </>
      ...
      <div className="w-full max-w-lg relative z-10 py-10">
        <div className="text-center mb-8">
          <div className="inline-block border-2 border-white/10 rounded-full px-6 py-2 mb-4">
             <span className="font-black italic tracking-[0.3em] text-white text-2xl">UKRAINE RP</span>
          </div>
          <p className="text-text-muted text-xs uppercase tracking-widest">Світ твоїх можливостей</p>
        </div>

        <AnimatePresence mode="wait">
          {step === 1 && renderAuth()}
          {step === 2 && renderCharacterCreation()}
          {step === 3 && renderPassportPreview()}
        </AnimatePresence>
      </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-lg font-black text-white uppercase tracking-widest">Фото Персонажу</h3>
                </div>
                <button onClick={() => setShowPhotoModal(false)} className="p-2 hover:bg-white/5 rounded-full"><X className="w-5 h-5" /></button>
              </div>

              <div className="space-y-4 text-center">
                <div className="w-32 h-40 bg-secondary-dark rounded-2xl mx-auto overflow-hidden border border-border-dark relative">
                  {formData.passportPhoto ? (
                    <img src={formData.passportPhoto} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center"><Camera className="w-12 h-12 text-white/5" /></div>
                  )}
                  {loading && <div className="absolute inset-0 bg-black/60 flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin" /></div>}
                </div>

                <div className="flex flex-col gap-3 pt-4">
                  <button onClick={() => fileInputRef.current?.click()} disabled={loading} className="py-4 bg-ukraine-blue text-white rounded-2xl font-black text-xs uppercase tracking-widest">ЗМІНИТИ ФОТО</button>
                  {formData.passportPhoto && <button onClick={deletePhoto} disabled={loading} className="py-4 bg-red-500/10 text-red-500 rounded-2xl font-black text-xs uppercase tracking-widest">ВИДАЛИТИ</button>}
                  <button onClick={() => setShowPhotoModal(false)} className="py-4 bg-white/5 text-white rounded-2xl font-black text-xs uppercase tracking-widest">ЗАКРИТИ</button>
                </div>
              </div>
            </motion.div>
          </div>
